...
const input = (label,value,onchange,type='text') => { const el=document.createElement('label'); el.textContent=label; const control=document.createElement('input'); control.type=type; control.value=value ?? ''; control.onchange=()=>onchange(type==='number'?Number(control.value):control.value); el.append(control); return el; };
const select = (label,value,choices,onchange) => { const el=document.createElement('label'); el.textContent=label; const control=document.createElement('select'); control.innerHTML=choices.map(v=>`<option value="${v}" ${v===value?'selected':''}>${v}</option>`).join(''); control.onchange=()=>onchange(control.value); el.append(control); return el; };
...
function renderSamples() { const gauge=config.gauges[Number($('preview-gauge').value||0)]; const root=$('samples'); root.replaceChildren(); const samples=defaultSamples(gauge); Object.entries(samples).forEach(([name,value])=>root.append(input(name,value,v=>samples[name]=v,'number'))); root.dataset.samples=JSON.stringify(samples); }
function renderAll(openGaugeIndex=null) { normalizeConfig(); renderSources(); renderGauges(openGaugeIndex); syncRaw(); }
async function requestPreview() { try { const samples=JSON.parse($('samples').dataset.samples||'{}'); $('samples').querySelectorAll('input').forEach(el=>samples[el.parentElement.firstChild.textContent]=Number(el.value)); const response=await fetch('/api/preview',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({config,gaugeIndex:Number($('preview-gauge').value),samples})}); if(!response.ok){const body=await response.json();throw new Error(body.errors.join('\n'));} $('preview-image').src=URL.createObjectURL(await response.blob()); message('Preview rendered.'); } catch(error){message(error.message,true);} }
